...
# This combines both classification methods (LPA & HC) and language types (expressive & expressive)
...
lang_data_wins <- winsorize(lang_data, expressive_cols, by_group = "lpa_class", trim_pct = 0.05)
...
desc_table <- long_data %>%
  ...
  gt() %>%
  tab_header(
    title = "Descriptive Statistics of Expressive Language by LPA Class",
...
save_as_docx(descriptive_flextable, path = "tables/supplementary/LPA_Expressive_descriptive_statistics.docx")
...
p1 <- ggplot(long_data, aes(x = timepoint, y = expressive, color = lpa_class)) +
  ...
  labs(
    title = "Language Expressive Trajectories by LPA Class",
    x = "Age (months)",
...
p2 <- ggplot(long_data, aes(x = timepoint, y = expressive, group = subject, color = lpa_class)) +
...
p3 <- ggplot(long_data, aes(x = expressive, y = timepoint_factor, fill = lpa_class)) +
  ...
  labs(
    title = "Distribution of Expressive Language by Age and LPA Class",
...
# Save the plots
ggsave("figures/main/LPA_Expressive_mean_trajectories.pdf", p1, width = 10, height = 7)
ggsave("figures/supplementary/LPA_Expressive_individual_trajectories.pdf", p2, width = 10, height = 7)
ggsave("figures/supplementary/LPA_Expressive_distribution_by_timepoint.pdf", p3, width = 10, height = 7)
ggsave("figures/supplementary/LPA_Expressive_combined_trajectory_plots.pdf", trajectory_plots, width = 10, height = 14)
# ============================================================================ #
...
run_main_models <- function(data) {
  ...
  data_std <- data %>%
    mutate(across(c(expressive, timepoint_centered, nonverbal_iq_6), 
                  ~scale(.) %>% as.vector(), 
                  .names = "{.col}_std"))
  ...
  m2 <- lmer(expressive ~ timepoint_centered + lpa_class + 
               (0 + timepoint_centered | subject),  # Random slopes only
             data = data, REML = FALSE,
             control = lmerControl(optimizer = "bobyqa"))
  
  # 4. Add LPA class effect on slope (interaction)
  m3 <- lmer(expressive ~ timepoint_centered * lpa_class + 
               (0 + timepoint_centered | subject),  # Random slopes only
             data = data, REML = FALSE,
             control = lmerControl(optimizer = "bobyqa"))
  ...
  m1_std <- lmer(expressive_std ~ timepoint_centered_std + 
                   (0 + timepoint_centered_std | subject),
                 data = data_std, REML = FALSE,
                 control = lmerControl(optimizer = "bobyqa"))
  
  m2_std <- lmer(expressive_std ~ timepoint_centered_std + lpa_class + 
                   (0 + timepoint_centered_std | subject),
                 data = data_std, REML = FALSE,
                 control = lmerControl(optimizer = "bobyqa"))
  
  m3_std <- lmer(expressive_std ~ timepoint_centered_std * lpa_class + 
                   (0 + timepoint_centered_std | subject),
                 data = data_std, REML = FALSE,
                 control = lmerControl(optimizer = "bobyqa"))
  
  m4_std <- lmer(expressive_std ~ timepoint_centered_std * lpa_class + 
...
gtsave(model_table, "tables/supplementary/LPA_Expressive_model_comparison.html")
...
save_as_docx(model_table_flex, path = "tables/supplementary/LPA_Expressive_model_comparison.docx")
...
gtsave(std_beta_gt, "tables/supplementary/LPA_Expressive_standardized_betas.html")
...
gtsave(r2_gt, "tables/supplementary/LPA_Expressive_r_squared.html")
...
save_as_docx(r2_flex, path = "tables/supplementary/LPA_Expressive_r_squared.docx")
...
std_beta_flex <- flextable(as.data.frame(std_beta_gt$`_data`)) %>% 
  ...
  align(align = "center", part = "all")
std_beta_flex
save_as_docx(std_beta_flex, path = "tables/supplementary/LPA_Expressive_standardized_betas.docx")
...
pdf("figures/supplementary/LPA_Expressive_model_diagnostics.pdf", width = 8, height = 10)
...
# Save tables
gtsave(emm_table, "tables/supplementary/LPA_Expressive_estimated_marginal_means.html")
gtsave(pairs_table, "tables/supplementary/LPA_Expressive_pairwise_comparisons.html")
gtsave(slopes_table, "tables/supplementary/LPA_Expressive_slope_comparisons.html")
...
save_as_docx(emm_flex, path = "tables/supplementary/LPA_Expressive_estimated_marginal_means.docx")
save_as_docx(pairs_flex, path = "tables/supplementary/LPA_Expressive_pairwise_comparisons.docx")
save_as_docx(slopes_flex, path = "tables/supplementary/LPA_Expressive_slope_comparisons.docx")
...
emm_plot <- ggplot(emm_lpa_time_df, aes(x = timepoint, y = emmean, color = lpa_class)) +
  ...
  labs(
    title = "Estimated Expressive Language Trajectories by LPA Class",
...
emm_plot
# Save the plot
ggsave("figures/supplementary/LPA_Expressive_estimated_trajectories.pdf", emm_plot, width = 10, height = 8)
...
gtsave(fixed_effects_table, "tables/main/LPA_Expressive_fixed_effects_table.html")
fixed_effects_flex <- gt_to_flextable(fixed_effects_table)
save_as_docx(fixed_effects_flex, path = "tables/main/LPA_Expressive_fixed_effects_table.docx")
...
gtsave(var_comp_table, "tables/supplementary/LPA_Expressive_variance_components.html")
var_comp_flex <- gt_to_flextable(var_comp_table)
save_as_docx(var_comp_flex, path = "tables/supplementary/LPA_Expressive_variance_components.docx")
...
ggsave("figures/supplementary/LPA_Expressive_forest_plot.pdf", forest_plot, width = 10, height = 8)
...
gtsave(summary_table, "tables/supplementary/LPA_Expressive_summary_findings.html")
summary_flex <- gt_to_flextable (summary_table)
save_as_docx(summary_flex, path = "tables/supplementary/LPA_Expressive_summary_findings.docx")
...
lang_data_wins05 <- winsorize(lang_data, expressive_cols, by_group = "lpa_class", trim_pct = 0.05)
lang_data_wins10 <- winsorize(lang_data, expressive_cols, by_group = "lpa_class", trim_pct = 0.10)
...
run_quadratic_models <- function(data) {
  ...
  m_linear <- lmer(expressive ~ timepoint_centered * lpa_class + 
                     sex + site + group_type + nonverbal_iq_6 + 
  ...
  m_quad <- lmer(expressive ~ timepoint_centered * lpa_class + 
  ...
  # Add interaction between quadratic term and LPA class
  m_quad_int <- lmer(expressive ~ timepoint_centered * lpa_class + 
...
for (i in 1:10) {
  ...
  # Run best model
  imp_long_models[[i]] <- lmer(expressive ~ timepoint_centered * lpa_class + 
                                 sex + site + group_type + nonverbal_iq_6 + 
                                 timepoint_centered:nonverbal_iq_6 +
                                 (0 + timepoint_centered | subject),
  ...
  imp_long_models[[paste0("quad_", i)]] <- lmer(expressive ~ timepoint_centered * lpa_class + 
                                                  timepoint_centered_sq * lpa_class +
                                                  sex + site + group_type + nonverbal_iq_6 + 
                                                  timepoint_centered:nonverbal_iq_6 +
                                                  (0 + timepoint_centered | subject),
