...
# This combines both classification methods (HC & HC) and language types (expressive & expressive)
...
run_main_models <- function(data) {
  ...
  m2_std <- lmer(expressive_std ~ timepoint_centered_std + hc_class + 
                   (0 + timepoint_centered_std | subject),
                 data = data_std, REML = FALSE,
                 control = lmerControl(optimizer = "bobyqa"))
  
  m3_std <- lmer(expressive_std ~ timepoint_centered_std * hc_class + 
                   (0 + timepoint_centered_std | subject),
                 data = data_std, REML = FALSE,
                 control = lmerControl(optimizer = "bobyqa"))
  
  m4_std <- lmer(expressive_std ~ timepoint_centered_std * hc_class + 
...
gtsave(std_beta_gt, "tables/supplementary/HC_Expressive_standardized_betas.html")
...
gtsave(r2_gt, "tables/supplementary/HC_Expressive_r_squared.html")
...
save_as_docx(r2_flex, path = "tables/supplementary/HC_Expressive_r_squared.docx")
...
std_beta_flex <- flextable(as.data.frame(std_beta_gt$`_data`)) %>% 
  ...
  align(align = "center", part = "all")
std_beta_flex
save_as_docx(std_beta_flex, path = "tables/supplementary/HC_Expressive_standardized_betas.docx")
