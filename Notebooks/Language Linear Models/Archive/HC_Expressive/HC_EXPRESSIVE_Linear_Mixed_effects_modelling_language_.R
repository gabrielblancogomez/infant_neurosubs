...
# Check for missing data in expressive language measures
missing_summary <- sapply(lang_data[expressive_cols], function(x) sum(is.na(x)))
missing_percent <- round(missing_summary / nrow(lang_data) * 100, 1)
missing_df <- data.frame(
  Timepoint = expressive_cols,
...
desc_table <- long_data %>%
  ...
  gt() %>%
  tab_header(
    title = "Descriptive Statistics of Expressive Language by HC Class",
...
save_as_docx(descriptive_flextable, path = "tables/supplementary/HC_Expressive_descriptive_statistics.docx")
...
p1 <- ggplot(long_data, aes(x = timepoint, y = expressive, color = hc_class)) +
  ...
  labs(
    title = "Language Expressive Trajectories by HC Class",
    x = "Age (months)",
    y = "Expressive Language Score",
    color = "HC Class"
  )
p1
# Plot 2: Individual trajectories with smoothed group trends
p2 <- ggplot(long_data, aes(x = timepoint, y = expressive, group = subject, color = hc_class)) +
...
p3 <- ggplot(long_data, aes(x = expressive, y = timepoint_factor, fill = hc_class)) +
  ...
  labs(
    title = "Distribution of Expressive Language by Age and HC Class",
    x = "Expressive Language Score",
    y = "Age (months)",
    fill = "HC Class"
  )
...
# Create diagram of LPA classes based on ASD Diagnosis 
# Calculate group sizes for each diagnosis group × LPA class combination
group_sizes <- long_data %>% filter (!outcome== "") %>%  # Filter out NAs in outcome
  distinct(subject, outcome, hc_class) %>%  # Get unique subjects per group
  count(outcome, hc_class) %>%                 # Count subjects per group
  group_by(outcome) %>%                         # For each diagnosis group...
  mutate(facet_label = paste0(                          # Create facet label with:
    outcome, 
    " (n = ", sum(n), ")\n",                           # Total N for diagnosis group
    "LPA counts: ", 
    paste0(hc_class, " = ", n, collapse = ", ")       # Breakdown by LPA class
  )) %>%
  ungroup()

# Merge the facet labels back into your data
plot_data <- long_data %>% filter (!outcome== "") %>% 
  left_join(group_sizes, by = c("outcome", "hc_class"))




p4 <-   ggplot(plot_data, 
               aes(x = timepoint, y = expressive, color = hc_class)) +
  stat_summary(fun = mean, geom = "point", size = 3) +
  stat_summary(fun = mean, geom = "line", aes(group = hc_class), size = 1.2) +
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.5) +
  scale_color_manual(values = hc_palette) +
  facet_wrap(~ facet_label) +  # Use the custom facet labels with group sizes
  theme_minimal() +
  theme(
    legend.position = "bottom",
    panel.grid.minor = element_blank(),
    text = element_text(size = 12),
    strip.text = element_text(size = 10, lineheight = 1.2)  # Adjust for multi-line labels
  ) +
  labs(
    title = "Language Receptive Trajectories by LPA Class and Diagnosis Group",
    x = "Age (months)",
...
# Save the plots
ggsave("figures/main/HC_Expressive_mean_trajectories.pdf", p1, width = 10, height = 7)
ggsave("figures/supplementary/HC_Expressive_individual_trajectories.pdf", p2, width = 10, height = 7)
ggsave("figures/supplementary/HC_Expressive_distribution_by_timepoint.pdf", p3, width = 10, height = 7)
ggsave("figures/supplementary/HC_Expressive_combined_trajectory_plots.pdf", trajectory_plots, width = 10, height = 14)
ggsave("figures/supplementary/HC_Expressive_combined_trajectory_plots_by_diagnosis.pdf", p4, width = 10, height = 14)
# ============================================================================ #
# 3. MIXED-EFFECTS MODELING - MAIN ANALYSIS
# ============================================================================ #

# Calculate the variance at the intercept level

variances_table <- long_data %>% filter(timepoint==6) %>%
  group_by(subject) %>%
  summarise(
    mean_expressive = mean(expressive, na.rm = TRUE),
    sd_expressive = sd(expressive, na.rm = TRUE)
  ) %>%
  summarise(
    var_intercept = var(mean_expressive, na.rm = TRUE),
    var_residual = var(sd_expressive, na.rm = TRUE)
  )

variances_table

# Calculate normaily of measures at timepoint 1
shapiro_test(long_data$expressive[long_data$timepoint==6])

# Plot expressive measures at timepoint 1
p5 <- ggplot(long_data %>% filter(timepoint==6), aes(x = expressive)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "lightblue", color = "black") +
  geom_density(alpha = 0.2, fill = "blue") +
  theme_minimal() +
  labs(
    title = "Distribution of Expressive Language at 6 Months",
    x = "Expressive Language Score",
    y = "Density"
  )
p5

# Calculate standard deviation and variance at timepoint 1
sd_6mo <- sd(long_data$expressive[long_data$timepoint==6], na.rm = TRUE)
sd_6mo
var_6mo <- var(long_data$expressive[long_data$timepoint==6], na.rm = TRUE)
var_6mo
# Calcualte kurtosis, skewness and normality
kurt_6mo<- kurtosis(long_data$expressive[long_data$timepoint==6], na.rm = TRUE)
kurt_6mo
skew_6mo <- skew(long_data$expressive[long_data$timepoint==6], na.rm = TRUE)
skew_6mo
norm_6mo <- shapiro.test(long_data$expressive[long_data$timepoint==6])
norm_6mo
# Check how many observations per subject
n_per_subject <- long_data %>% drop_na(expressive) %>%
  group_by(subject) %>%
  summarise(n = n()) %>%
  group_by(n) %>%
  summarise(obs_per_subject = n()) 
n_per_subject
...
m0 <- lmer(expressive ~ 1 + (1 | subject), data = long_data, REML = FALSE)

m0

m1 <- lmer(expressive ~ timepoint_centered * hc_class +  
             (1 | subject),  # Random slopes only
           data = long_data, REML = FALSE,
           control = lmerControl(optimizer = "bobyqa"))

m1

optimizer_change <- lmer(expressive ~ 1 + (1 | subject), data = long_data, 
              control = lmerControl(optimizer = "bobyqa"))
optimizer_change
# Define function to run main model sequence
run_main_models <- function(data) {
  # 1. Unconditional means model (random intercept only)
  m0 <- lmer(expressive ~ 1 + (1 | subject), data = data, REML = FALSE)
  
  # 2. Unconditional growth model (random slope only for convergence)
  m1 <- lmer(expressive ~ timepoint_centered + 
               (0 + timepoint_centered | subject),  # Random slopes only
             data = data, REML = FALSE,
             control = lmerControl(optimizer = "bobyqa"))
  
  # 3. Add HC class effect on intercept
  m2 <- lmer(expressive ~ timepoint_centered + hc_class + 
               (0 + timepoint_centered | subject),  # Random slopes only
             data = data, REML = FALSE,
             control = lmerControl(optimizer = "bobyqa"))
  
  # 4. Add HC class effect on slope (interaction)
  m3 <- lmer(expressive ~ timepoint_centered * hc_class + 
               (0 + timepoint_centered | subject),  # Random slopes only
             data = data, REML = FALSE,
             control = lmerControl(optimizer = "bobyqa"))
  
  # 5. Add covariates (sex, site, risk, iq)
  m4 <- lmer(expressive ~ timepoint_centered * hc_class + 
...
# Filter out NAs in nonverbal_iq_6 for model fitting
#main_data <- long_data %>% filter(!is.na(nonverbal_iq_6))
main_data <- long_data
# Run the main models
main_models <- run_main_models(main_data)
main_models
...
model_table <- model_comp %>%
  gt() %>%
  tab_header(title = "Model Comparison for Expressive Language Development") %>%
...
gtsave(model_table, "tables/supplementary/HC_Expressive_model_comparison.html")
# Convert to flextable by extracting the data
model_data <- as.data.frame(model_table$`_data`)  # Extract underlying data
model_table_flex <- flextable(model_data) %>% 
  set_table_properties(layout = "autofit") %>%
  theme_zebra() %>%
  align(align = "center", part = "all")
save_as_docx(model_table_flex, path = "tables/supplementary/HC_Expressive_model_comparison.docx")
...
pdf("figures/supplementary/HC_Expressive_model_diagnostics.pdf", width = 8, height = 10)
...
# Save tables
gtsave(emm_table, "tables/supplementary/HC_Expressive_estimated_marginal_means.html")
gtsave(pairs_table, "tables/supplementary/HC_Expressive_pairwise_comparisons.html")
gtsave(slopes_table, "tables/supplementary/HC_Expressive_slope_comparisons.html")
...
save_as_docx(emm_flex, path = "tables/supplementary/HC_Expressive_estimated_marginal_means.docx")
save_as_docx(pairs_flex, path = "tables/supplementary/HC_Expressive_pairwise_comparisons.docx")
save_as_docx(slopes_flex, path = "tables/supplementary/HC_Expressive_slope_comparisons.docx")
...
emm_plot <- ggplot(emm_hc_time_df, aes(x = timepoint, y = emmean, color = hc_class)) +
  ...
  labs(
    title = "Estimated Expressive Language Trajectories by HC Class",
    subtitle = "With 95% confidence intervals",
    x = "Age (months)",
    y = "Estimated Expressive Language Score",
    color = "HC Class",
    fill = "HC Class"
  )
emm_plot
# Save the plot
ggsave("figures/supplementary/HC_Expressive_estimated_trajectories.pdf", emm_plot, width = 10, height = 8)
...
gtsave(fixed_effects_table, "tables/main/HC_Expressive_fixed_effects_table.html")
fixed_effects_flex <- gt_to_flextable(fixed_effects_table)
save_as_docx(fixed_effects_flex, path = "tables/main/HC_Expressive_fixed_effects_table.docx")
...
gtsave(var_comp_table, "tables/supplementary/HC_Expressive_variance_components.html")
var_comp_flex <- gt_to_flextable(var_comp_table)
save_as_docx(var_comp_flex, path = "tables/supplementary/HC_Expressive_variance_components.docx")
...
ggsave("figures/supplementary/HC_Expressive_forest_plot.pdf", forest_plot, width = 10, height = 8)
...
summary_points <- data.frame(
  ...
  Description = c(
    ...
    paste0("The final model explained ", 
           ...
           "% of the variance in expressive language."),
...
gtsave(summary_table, "tables/supplementary/HC_Expressive_summary_findings.html")
summary_flex <- gt_to_flextable (summary_table)
save_as_docx(summary_flex, path = "tables/supplementary/HC_Expressive_summary_findings.docx")
...
lang_data_wins05 <- winsorize(lang_data, expressive_cols, by_group = "hc_class", trim_pct = 0.05)
lang_data_wins10 <- winsorize(lang_data, expressive_cols, by_group = "hc_class", trim_pct = 0.10)
...
create_model_table <- function(model_list, title) {
  model_names <- names(model_list)
  
  # Create appropriate descriptions based on model type
  descriptions <- if(length(model_list) == 7) {
    c("Random intercept only",
      "Random slope only",
      "Fixed HC effect",
      "HC*Time interaction",
      "Covariates (sex, site, risk)",
      "Nonverbal IQ",
      "Nonverbal IQ*Time")
  } else if(length(model_list) == 3) {
    c("Linear time model",
      "Quadratic time term",
      "Quadratic*HC interaction")
  } else {
    rep("Model", length(model_list))
  }
  
  # Extract AIC, BIC, logLik
  aic_values <- sapply(model_list, AIC)
  bic_values <- sapply(model_list, BIC)
  loglik_values <- sapply(model_list, logLik)
  
  # Check convergence
  convergence_status <- sapply(model_list, check_convergence)
...
# Extract fixed effects from models with different winsorization levels
...
run_quadratic_models <- function(data) {
  ...
  m_linear <- lmer(expressive ~ timepoint_centered * hc_class + 
                     sex + site + group_type + nonverbal_iq_6 + 
  ...
  m_quad <- lmer(expressive ~ timepoint_centered * hc_class + 
  ...
  m_quad_int <- lmer(expressive ~ timepoint_centered * hc_class + 
...
# ============================================================================ #
# Prepare long data for imputation
long_imp_prep <- long_data %>%
  mutate(timepoint_centered_sq = timepoint_centered^2)
...
for (i in 1:10) {
  ...
  # Run best model
  imp_long_models[[i]] <- lmer(expressive ~ timepoint_centered * hc_class + 
                                 sex + site + group_type + nonverbal_iq_6 + 
                                 timepoint_centered:nonverbal_iq_6 +
                                 (0 + timepoint_centered | subject),
  ...
  imp_long_models[[paste0("quad_", i)]] <- lmer(expressive ~ timepoint_centered * hc_class + 
                                                  timepoint_centered_sq * hc_class +
                                                  sex + site + group_type + nonverbal_iq_6 + 
                                                  timepoint_centered:nonverbal_iq_6 +
                                                  (0 + timepoint_centered | subject),
...
# Function to pool lmer models
pool_lmer_results <- function(model_list) {
  ...
  results <- lapply(model_list, function(x) {
    tidy(x, conf.int = TRUE) %>%
      mutate(
        ...
        p.value = if ("p.value" %in% names(.)) p.value else 
          2 * pnorm(abs(statistic), lower.tail = FALSE)
      ) %>%
      select(term, estimate, std.error, statistic, p.value, conf.low, conf.high)
  })
  
  # Combine results for pooling
  combined <- bind_rows(results, .id = "imputation")
  ...
  pooled <- combined %>%
    ...
    summarize(
      ...
      std.error = sqrt(mean(std.error^2) + var(estimate)),
...
pooled_linear <- pool_lmer_results(linear_models_lme4)
pooled_quad <- pool_lmer_results(quad_models_lme4)

# View results
print(pooled_linear)
print(pooled_quad)
# Save pooled results
capture.output(summary(pooled_linear), file = "sensitivity/mice_pooled_linear.txt")
capture.output(summary(pooled_quad), file = "sensitivity/mice_pooled_quadratic.txt")
