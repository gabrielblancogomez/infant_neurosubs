...
# This combines both classification methods (LPA & HC) and language types (receptive & receptive)
...
# Select Palettes
lpa_palette <- c("#184e77","#7570b3", "#1b9e77")
...
lang_data_wins <- winsorize(lang_data, receptive_cols, by_group = "lpa_class", trim_pct = 0.05)
...
desc_table <- long_data %>%
  ...
  pivot_wider(
    id_cols = timepoint,
    names_from = lpa_class,
    values_from = c(n, mean, sd),
    names_sep = "_"
  ) %>%
  gt() %>%
  tab_header(
    title = "Descriptive Statistics of Receptive Language by LPA Class",
...
gtsave(desc_table, "tables/supplementary/LPA_descriptive_statistics.html")
...
save_as_docx(descriptive_flextable, path = "tables/supplementary/LPA_Receptive_descriptive_statistics.docx")
...
p1 <- ggplot(long_data, aes(x = timepoint, y = receptive, color = lpa_class)) +
  ...
  scale_color_manual(values = lpa_palette) +
  theme_minimal() +
  theme(
...
p3 <- ggplot(long_data, aes(x = receptive, y = timepoint_factor, fill = lpa_class)) +
  ...
  scale_fill_manual(values = lpa_palette) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    panel.grid.minor = element_blank(),
...
group_sizes <- long_data %>% filter (!outcome== "") %>%  # Filter out NAs in outcome
  distinct(subject, outcome, lpa_class) %>%  # Get unique subjects per group
  count(outcome, lpa_class) %>%                 # Count subjects per group
  group_by(outcome) %>%                         # For each diagnosis group...
  mutate(facet_label = paste0(                          # Create facet label with:
    outcome, 
    " (n = ", sum(n), ")\n",                           # Total N for diagnosis group
    "LPA counts: ", 
    paste0(lpa_class, " = ", n, collapse = ", ")       # Breakdown by LPA class
  )) %>%
  ungroup()

# Merge the facet labels back into your data
plot_data <- long_data %>% filter (!outcome== "") %>% 
  left_join(group_sizes, by = c("outcome", "lpa_class"))




p4 <-   ggplot(plot_data, 
               aes(x = timepoint, y = receptive, color = lpa_class)) +
  stat_summary(fun = mean, geom = "point", size = 3) +
  stat_summary(fun = mean, geom = "line", aes(group = lpa_class), size = 1.2) +
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.5) +
  scale_color_manual(values = lpa_palette) +
...
p5 <-   ggplot(long_data, 
  ...
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.5) +
  scale_color_manual(values = lpa_palette) +
  facet_wrap(~ sex) +  # Use the custom facet labels with group sizes
  theme_minimal() +
  theme(
    legend.position = "bottom",
    panel.grid.minor = element_blank(),
  ...
  labs(
    title = "Language Receptive Trajectories by LPA Class and Sex",
    x = "Age (months)",
    y = "Receptive Language Score",
    color = "LPA Class"
  )
...
p6  <-   ggplot(long_data, 
  ...
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.5) +
  scale_color_manual(values = lpa_palette) +
  facet_wrap(~ group_type) +  # Use the custom facet labels with group sizes
  theme_minimal() +
  theme(
    legend.position = "bottom",
    panel.grid.minor = element_blank(),
  ...
  labs(
    title = "Language Receptive Trajectories by LPA Class and ASD Likelihood",
    x = "Age (months)",
    y = "Receptive Language Score",
    color = "LPA Class"
  )
...
p7  <-   ggplot(long_data, 
  ...
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.5) +
  scale_color_manual(values = lpa_palette) +
  facet_wrap(~ site) +  # Use the custom facet labels with group sizes
  theme_minimal() +
  theme(
    legend.position = "bottom",
    panel.grid.minor = element_blank(),
  ...
  labs(
    title = "Language Receptive Trajectories by LPA Class and Site",
    x = "Age (months)",
    y = "Receptive Language Score",
    color = "LPA Class"
  )
...
# Save the plots
ggsave("figures/main/LPA_Receptive_mean_trajectories.pdf", p1, width = 10, height = 7)
ggsave("figures/supplementary/LPA_Receptive_individual_trajectories.pdf", p2, width = 10, height = 7)
ggsave("figures/supplementary/LPA_Receptive_distribution_by_timepoint.pdf", p3, width = 10, height = 7)
ggsave("figures/supplementary/LPA_Receptive_combined_trajectory_plots.pdf", trajectory_plots, width = 10, height = 14)
# ============================================================================ #
...
run_main_models <- function(data) {
  ...
  # 3. Add LPA class effect on intercept
  m2 <- lmer(receptive ~ timepoint_centered + lpa_class + 
               (0 + timepoint_centered | subject),  # Random slopes only
             data = data, REML = FALSE,
             control = lmerControl(optimizer = "bobyqa"))
  
  # 4. Add LPA class effect on slope (interaction)
  m3 <- lmer(receptive ~ timepoint_centered * lpa_class + 
               (0 + timepoint_centered | subject),  # Random slopes only
             data = data, REML = FALSE,
             control = lmerControl(optimizer = "bobyqa"))
  
  # 5. Add covariates (sex, site, risk, iq)
  m4 <- lmer(receptive ~ timepoint_centered * lpa_class + 
               sex + site + group_type + nonverbal_iq_6+
               (0 + timepoint_centered | subject),  # Random slopes only
             data = data, REML = FALSE,
             control = lmerControl(optimizer = "bobyqa"))
  
  # 6. Add interaction between sex and LPA
  m5 <- lmer(receptive ~ timepoint_centered * lpa_class *sex + 
               site + group_type + nonverbal_iq_6+
               (0 + timepoint_centered | subject),  # Random slopes only
             data = data, REML = FALSE,
             control = lmerControl(optimizer = "bobyqa"))
   
  # Return list of models
  return(list(m0 = m0, m1 = m1, m2 = m2, m3 = m3, m4 = m4, m5=m5))
}
...
gtsave(model_table, "tables/supplementary/LPA_Receptive_model_comparison.html")
...
save_as_docx(model_table_flex, path = "tables/supplementary/LPA_Receptive_model_comparison.docx")
...
pdf("figures/supplementary/LPA_Receptive_model_diagnostics.pdf", width = 8, height = 10)
...
# Calculate estimated marginal means for LPA class by timepoint
emm_lpa_time <- emmeans(best_model, ~ lpa_class | timepoint_centered, 
                        at = list(timepoint_centered = c(0, 6, 12, 18, 24, 30)))

emm_lpa_time_later <- emmeans(best_model, ~ lpa_class | timepoint_centered, 
                        at = list(timepoint_centered = c(24, 30)))

# Convert back to original timepoint scale for plotting and interpretation
emm_lpa_time_df <- as.data.frame(emm_lpa_time) %>%
  mutate(timepoint = timepoint_centered + 6)

# Pairwise comparisons between LPA classes at each timepoint
pairs_lpa <- pairs(emm_lpa_time_later, adjust = "tukey")
pairs_lpa_df <- as.data.frame(pairs_lpa)

# Test for overall trend differences (slope contrasts)
emm_slopes <- emtrends(best_model, ~ lpa_class, var = "timepoint_centered")
...
emm_table <- emm_lpa_time_df %>%
  select(lpa_class, timepoint_centered, emmean, SE, lower.CL, upper.CL) %>%
  mutate(timepoint = timepoint_centered + 6) %>%
  arrange(timepoint, lpa_class) %>%
  gt() %>%
  tab_header(
    title = "Estimated Marginal Means by LPA Class and Timepoint",
...
pairs_table <- pairs_lpa_df %>%
  gt() %>%
  tab_header(
    title = "Pairwise Comparisons Between LPA Classes at Each Timepoint",
...
slopes_table <- slope_pairs_df %>%
  gt() %>%
  tab_header(
    title = "Differences in Developmental Trajectories Between LPA Classes",
...
# Save tables
gtsave(emm_table, "tables/supplementary/LPA_Receptive_estimated_marginal_means.html")
gtsave(pairs_table, "tables/supplementary/LPA_Receptive_pairwise_comparisons.html")
gtsave(slopes_table, "tables/supplementary/LPA_Receptive_slope_comparisons.html")
...
save_as_docx(emm_flex, path = "tables/supplementary/LPA_Receptive_estimated_marginal_means.docx")
save_as_docx(pairs_flex, path = "tables/supplementary/LPA_Receptive_pairwise_comparisons.docx")
save_as_docx(slopes_flex, path = "tables/supplementary/LPA_Receptive_slope_comparisons.docx")

# Plot estimated marginal means with custom palette
emm_plot <- ggplot(emm_lpa_time_df, aes(x = timepoint, y = emmean, color = lpa_class)) +
  geom_line(size = 1.2) +
  geom_ribbon(aes(ymin = lower.CL, ymax = upper.CL, fill = lpa_class), 
              alpha = 0.2, color = NA) +
  scale_color_manual(values = lpa_palette) +
  scale_fill_manual(values = lpa_palette) +
  theme_minimal() +
  ...
  labs(
    title = "Estimated Receptive Language Trajectories by LPA Class",
...
emm_plot
# Save the plot
ggsave("figures/supplementary/LPA_Receptive_estimated_trajectories.pdf", emm_plot, width = 10, height = 8)
...
gtsave(fixed_effects_table, "tables/main/LPA_Receptive_fixed_effects_table.html")
fixed_effects_flex <- gt_to_flextable(fixed_effects_table)
save_as_docx(fixed_effects_flex, path = "tables/main/LPA_Receptive_fixed_effects_table.docx")
...
gtsave(var_comp_table, "tables/supplementary/LPA_Receptive_variance_components.html")
var_comp_flex <- gt_to_flextable(var_comp_table)
save_as_docx(var_comp_flex, path = "tables/supplementary/LPA_Receptive_variance_components.docx")
...
ggsave("figures/supplementary/LPA_Receptive_forest_plot.pdf", forest_plot, width = 10, height = 8)

# Create a summary table of key findings
summary_points <- data.frame(
  Finding = c(
    "LPA Classes Trajectory Differences",
    "Nonverbal IQ Effect",
    "Model Fit Quality",
    "Missing Data Impact"
  ),
  Description = c(
    paste0("The three LPA classes showed ", 
...
summary_table <- summary_points %>%
  ...
  tab_header(
    ...
    subtitle = "Language development trajectories by LPA Class"
  ) %>%
  cols_label(
    Finding = "Key Finding",
...
gtsave(summary_table, "tables/supplementary/LPA_Receptive_summary_findings.html")
summary_flex <- gt_to_flextable (summary_table)
save_as_docx(summary_flex, path = "tables/supplementary/LPA_Receptive_summary_findings.docx")
...
lang_data_wins05 <- winsorize(lang_data, receptive_cols, by_group = "lpa_class", trim_pct = 0.05)
lang_data_wins10 <- winsorize(lang_data, receptive_cols, by_group = "lpa_class", trim_pct = 0.10)
...
quad_models <- run_quadratic_models(long_data %>% filter(!is.na(nonverbal_iq_6)))
...
for (i in 1:10) {
  ...
  # Run best model
  imp_long_models[[i]] <- lmer(receptive ~ timepoint_centered * lpa_class + 
                                 sex + site + group_type + nonverbal_iq_6 + 
  ...
  imp_long_models[[paste0("quad_", i)]] <- lmer(receptive ~ timepoint_centered * lpa_class + 
                                                  timepoint_centered_sq * lpa_class +
                                                  sex + site + group_type + nonverbal_iq_6 + 
                                                  timepoint_centered:nonverbal_iq_6 +
                                                  (0 + timepoint_centered | subject),
                                                data = imp_data, REML = FALSE,
